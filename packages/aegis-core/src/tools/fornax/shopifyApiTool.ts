...
class ShopifyCreateProductTool extends Tool {
  ...
  protected async _call(input: string): Promise<string> {
    ...
    const createdProductsLog: string[] = [];

    let productsToCreate: z.infer<typeof productDataSchema>[];
    ...
    for (const product of productsToCreate) {
      try {
        ...
        if (axios.isAxiosError(error)) { errorMessage += ` Details: ${JSON.stringify(error.response?.data)}`; }
    ...
    if (createdProductsLog.length === 0) return "No products were created.";
...
class ShopifyGetOrdersTool extends Tool {
  ...
  protected async _call(input: string): Promise<string> {
    ...
    try {
      ...
      if (inputJson.status) params.status = inputJson.status;
    } catch (e) {
    ...
    try {
      ...
      if (response.status === 200) {
        if (response.data.orders.length === 0) return `No orders found with status: '${params.status}'.`;
      ...
      if (axios.isAxiosError(error)) return `Error getting orders from Shopify: ${JSON.stringify(error.response?.data)}`;
