...
class MarketAnalysisTool extends Tool {
  ...
  private searchTool = new TavilySearchResults({ maxResults: 5 });
  private llm = new ChatGoogleGenerativeAI({ model: 'gemini-2.5-flash' });

  protected async _call(keyword: string | undefined): Promise<string> {
    if (!keyword) {
