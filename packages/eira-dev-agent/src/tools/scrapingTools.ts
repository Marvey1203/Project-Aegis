...
type BasicPuppeteerScrapeToolInput = z.infer<typeof basicToolInputSchema>;

async function basicScrapePage(args: BasicPuppeteerScrapeToolInput): Promise<string> {
...
type AdvancedPuppeteerScrapeToolInput = z.infer<typeof advancedToolInputSchema>;

async function advancedScrapePage(args: AdvancedPuppeteerScrapeToolInput): Promise<string> {
  let browser: Browser | null = null;
        ...
        try {
          switch (action.actionType) {
            case 'click':
              ...
              break;
            case 'type':
              if (!action.selector || typeof action.textToType !== 'string') throw new Error("Selector and textToType are required for type action.");
              await page.waitForSelector(action.selector, { timeout: action.timeout || operationTimeout - (Date.now() - startTime) });
            ...
            case 'waitForTimeout':
              ...
              break;
            case 'selectOption':
              if (!action.selector || typeof action.valueToSelect !== 'string') throw new Error("Selector and valueToSelect are required for selectOption action.");
              await page.waitForSelector(action.selector, { timeout: action.timeout || operationTimeout - (Date.now() - startTime) });
              await page.select(action.selector, action.valueToSelect);
              break;
            case 'scrollToElement':
              if (!action.selector) throw new Error("Selector is required for scrollToElement action.");
              await page.waitForSelector(action.selector, { timeout: action.timeout || operationTimeout - (Date.now() - startTime) });
            ...
            case 'focus':
              ...
              break;
            default:
              console.warn(`Unknown action type: ${(action as any).actionType}`);
          }
        } catch (actError: any) {
        ...
        try {
          switch (ext.extractType) {
            ...
            case 'html':
              ...
              break;
            case 'attribute':
              if (!ext.attributeName) throw new Error("attributeName is required for extractType 'attribute'.");
              extractedData[ext.name] = await page.$eval(ext.selector, (el, attr) => el.getAttribute(attr)?.trim() || null, ext.attributeName as string);
              break;
            case 'count':
            ...
            case 'list_html':
              ...
              break;
            case 'list_attribute':
              if (!ext.attributeName) throw new Error("attributeName is required for extractType 'list_attribute'.");
              extractedData[ext.name] = await page.$$eval(ext.selector, (els, attr) => els.map(el => el.getAttribute(attr)?.trim() || ''), ext.attributeName as string);
              break;
            default:
